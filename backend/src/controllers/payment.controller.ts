...
export const PaymentController = {
  ...
  async getAll(_req: AuthRequest, res: Response) {
    try {
      ...
      res.status(500).json({ message: 'Échec de la récupération des paiements' });
    }
  },
  ...
  async create(req: AuthRequest, res: Response) {
    try {
      ...
      if (!courseId || typeof courseId !== 'string') {
        return res.status(400).json({ message: 'L\'identifiant du cours est requis' });
      }
      ...
      res.status(500).json({ message: 'Échec de la création du paiement' });
    }
  },
};
