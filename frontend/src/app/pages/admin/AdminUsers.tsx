...
export function AdminUsers() {
  ...
  const openEditModal = (u: ApiUser) => {
    ...
    if (u.role.toLowerCase() === 'teacher') {
      ...
      usersApi.getEligibleCourses(u.id)
        .then((courses: any) => {
          setEligibleCourses(courses);
          const assigned = (courses as { id: string; teacherId: string | null }[])
            .filter(c => c.teacherId === u.id)
            .map(c => c.id);
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <button type="button" onClick={() => setTeacherForm(p => ({ ...p, showPassword: !p.showPassword }))}
                    ...
                    {teacherForm.showPassword ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                  </button>
                </div>
                <p className="text-xs text-muted-foreground mt-1">Le formateur devra changer ce mot de passe à sa première connexion.</p>
              </div>
              <div className="flex justify-end gap-3 pt-2">
